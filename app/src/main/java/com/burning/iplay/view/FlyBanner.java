...
public class FlyBanner extends RelativeLayout {
    ...
    private List<RecommendBanner.InfoBean> mDatas;
     ...
     */
    public void setDatas(List<RecommendBanner.InfoBean> datas) {
        mDatas = datas;
        //加载网络图片
        mIsImageUrl = true;
        if (datas.size() <= 1)
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            if (mIsImageUrl) {
                /*Picasso.with(getContext())
                        ...
                        .into(imageView);*/
                ImageLoader.loadImage(getContext(), mDatas.get(toRealPosition(position)).getImgUrl(), imageView);
            } else {
    ...
    public interface OnItemClickListener {
        void onItemClick(int position, RecommendBanner.InfoBean infoBean);
    }
}